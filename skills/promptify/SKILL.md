...
4. **Ask goal-vs-normal — only if the harness is Claude Code.** Explicitly ask the user whether they want a `/goal` completion condition or a normal prompt, per [references/goal-vs-normal.md](references/goal-vs-normal.md). Codex CLI always gets a normal prompt, never a fabricated goal construct — see that file's "Why Codex CLI never gets a /goal" section for the reasoning.
